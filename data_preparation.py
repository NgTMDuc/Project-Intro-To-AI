...
if os.path.isfile(file_name) == False:
    response = requests.get(
        "http://planetpixelemporium.com/download/download.php?5672/mars_12k_topo.jpg")
    # Create a file with the name above and save the content of response to it
...
for size in img_size:
    ...
    while i <= num_bins * bin_size:
        ...
        tmp_data[(i - bin_size, i)] = [arr[0]
                                       for arr in arrays[size] if arr[1] < i]
